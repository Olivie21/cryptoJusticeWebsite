...
import Practices from "../../pages/api/Practices";
import Cases from "../../pages/api/case";
...
const BlogSidebar = (props) => {
  ...
  return (
    <div className={`col-span-4 md:col-span-12 ${props.blLeft}`}>
      <div
        className={`pl-[45px] lg:pl-[35px] md:pl-0 md:mt-[80px] sm:mt-[70px] md:max-w-[400px] ${props.LeftClass}`}
      >
        <form onSubmit={SubmitHandler}>
          <div className="relative">
            <input
              type="text"
              className="form-control w-full text-[16px] border border-[#ddd] h-[50px]  pl-[10px] focus:outline-0 focus:shadow-none  text-[#666]"
              placeholder="Search Post.."
            />
            <button
              type="submit"
              className="text-[20px] text-white  bg-[#282e3f] absolute right-0 top-[52%] h-[50px]
        ...
        <div className="mt-[65px] pt-[15px] px-[30px] pb-[20px] bg-[#f5f5f5]">
          <h3
            className="text-[25px] relative text-[#282e3f] text-left capitalize pb-[20px] font-medium
                        "
          >
            Recent Case
          </h3>
                  ...
                  <h4>
                    <Link
                      ...
                      className="inline-block font-base-font font-medium
                                    text-[#666] transition-all hover:text-[#c0b596]"
                    >
                      {cases.cTitle} the great explorer of the truth,
                    </Link>
                  </h4>
                </div>
              </div>
        ...
        <div className="mt-[65px]">
          ...
          <ul className="mt-[25px] overflow-hidden">
            <li className="float-left mr-[8px] mb-[8px]">
              <Link
                onClick={ClickHandler}
                href="/blog"
                className="bg-[#f5f5f5] inline-block py-[10px] px-[20px] text-[14px] text-[#333] transition-all hover:bg-[#c0b596] hover:text-white"
              >
                Attorney
              </Link>
            </li>
        ...
        <div className="mt-[45px]">
          <h3 className="text-[25px] relative text-[#282e3f] text-left capitalize  font-medium">
            Instagram
          </h3>
          <ul className="flex flex-wrap mt-[30px]">
            {Practices.slice(0, 6).map((practice, i) => (
              <li
                className="max-w-[33.33%] mb-[5px] px-[3px] flex-[0,0,33.33%]"
                key={i}
              >
                <Link
                  onClick={ClickHandler}
                  ...
                  as={`/practice-single/${practice.slug}`}
                >
                  <Image className="w-full" src={practice.sImg} alt="" />
                </Link>
              </li>
            ))}
          </ul>
